...
如果你已经有了`.button.active` 选择器在代码里了，那么重新定义`.header.button`的特异性和联合修饰符的选择器的特异性是完全相同的，这取决于CSS规则声明的顺序。然而为了防备使用了有前缀的修饰符，你可以一直确使使用了层叠的选择器`.header .button`会覆盖`.button--active`修饰符。
